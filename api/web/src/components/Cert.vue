...
const is_img = computed(() => {
    return asset.value && asset.value.name && (
        asset.value.name.toLowerCase().endsWith('.jpg')
...
const is_pdf = computed(() => {
    return asset.value && asset.value.name && asset.value.name.endsWith('.pdf')
})

const preview = computed(() => {
    if (!cert.value.asset) return '';
    const url = window.stdurl(`/api/asset/${cert.value.asset}/raw`);
...
const fetch = async () => {
    loading.cert = true;
    cert.value = await window.std(`/api/user/${route.params.userid}/cert/${route.params.certid}`);
    asset.value = await window.std(`/api/asset/${cert.value.asset}`);
    loading.cert = false;
}
